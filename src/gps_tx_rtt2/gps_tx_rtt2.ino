...
void setup()
{
  ...
  myGNSS.setNavigationFrequency(100); //Produce two solutions per second
...
void loop()
{
  ...
  if (myGNSS.packetUBXNAVPVT->automaticFlags.flags.bits.callbackCopyValid == true)
  {
    ...
    Serial.println();
    rf95.send((uint8_t *)&(myGNSS.packetUBXNAVPVT->callbackData), sizeof(myGNSS.packetUBXNAVPVT->callbackData));
